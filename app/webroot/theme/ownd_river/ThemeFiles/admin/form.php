...
<div class="submit">
	<?php if ($this->request->action == 'admin_add'): ?>
		<?php $this->BcBaser->link('一覧に戻る', array_merge(array('action' => 'index', $theme, $plugin, $type), $params), array('class' => 'btn-gray button bca-btn')); ?>
	...
	<?php elseif ($this->request->action == 'admin_edit'): ?>
		...
		<?php if($isWritable): ?>
			...
			<?php $this->BcBaser->link('削除', array_merge(array('action' => 'del', $theme, $plugin, $type), $params), array('class' => 'submit-token button bca-btn'), sprintf('%s を本当に削除してもいいですか？', basename($path)), false) ?>
		<?php endif ?>	
	<?php else: ?>
		<?php $this->BcBaser->link('一覧に戻る', array_merge(array('action' => 'index', $theme, $plugin, $type), explode('/', dirname($path))), array('class' => 'btn-gray button bca-btn', 'data-bca-btn-type' => 'back-to-list'F)); ?>
