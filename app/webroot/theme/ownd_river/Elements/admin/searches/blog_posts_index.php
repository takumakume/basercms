...
<p class="bca-search__input-list">
	...
	<?php if ($this->BlogCategories): ?>
		<span class="bca-search__input-item"><?php echo $this->BcForm->label('BlogPost.blog_category_id', __d('baser', 'カテゴリー')) ?> <?php echo $this->BcForm->input('BlogPost.blog_category_id', array('type' => 'select', 'options' => $this->BlogCategories, 'escape' => false, 'empty' => __d('baser', '指定なし'))) ?></span>
	<?php endif ?>
	<?php if ($blogContent['BlogContent']['tag_use'] && $this->BlogTags): ?>
		<span class="bca-search__input-item"><?php echo $this->BcForm->label('BlogPost.blog_tag_id', __d('baser', 'タグ')) ?> <?php echo $this->BcForm->input('BlogPost.blog_tag_id', array('type' => 'select', 'options' => $this->BlogTags, 'escape' => false, 'empty' => __d('baser', '指定なし'))) ?></span>
	<?php endif ?>
	<span class="bca-search__input-item"><?php echo $this->BcForm->label('BlogPost.status', __d('baser', '公開状態')) ?> <?php echo $this->BcForm->input('BlogPost.status', array('type' => 'select', 'options' => $this->BcText->booleanMarkList(), 'empty' => __d('baser', '指定なし'))) ?></span>
	<span class="bca-search__input-item"><?php echo $this->BcForm->label('BlogPost.user_id', __d('baser', '作成者')) ?> <?php echo $this->BcForm->input('BlogPost.user_id', array('type' => 'select', 'options' => $users, 'empty' => __d('baser', '指定なし'))) ?></span>
</p>
