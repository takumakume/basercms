...
<?php endif ?>
	...
	<form action="<?php echo $this->request->base ?>/installations/step2" method="post" id="checkenv">
		...
		<div class="submit">
			<?php echo $this->BcForm->button(__d('baser', '再チェック'), array('class' => 'btn-orange button', 'id' => 'btncheckagain')) ?>
<?php if (!$blRequirementsMet): ?>
			<?php echo $this->BcForm->button(__d('baser', '次のステップへ'), array('class' => 'btn-red button', 'id' => 'btnnext', 'style' => 'display:none')) ?>
<?php else: ?>
			<?php echo $this->BcForm->button(__d('baser', '次のステップへ'), array('class' => 'btn-red button', 'id' => 'btnnext')) ?>
<?php endif ?>
		</div>
	</form>
