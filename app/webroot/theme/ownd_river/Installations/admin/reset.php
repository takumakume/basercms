...
<?php if (!$complete): ?>
	...
	<?php if (BC_INSTALLED): ?>
		...
		<ul><li><?php $this->BcBaser->link(__d('baser', 'バックアップはこちらから'), array('admin' => true, 'controller' => 'tools', 'action' => 'maintenance', 'backup')) ?></li></ul>
	<?php endif ?>
	...
	<?php echo $this->BcForm->end(array('label' => __d('baser', '初期化する'), 'class' => 'button')) ?>
...
<?php else: ?>
	...
	<div class="submit">
		<?php $this->BcBaser->link(__d('baser', 'インストールページへ'), '/', array('class' => 'button btn-red')) ?>
	</div>
<?php endif ?>